<?php
	...
	if($_REQUEST["type"] == "RUNJOB") 
	{
		...
		// send email
		$aEmail = new AsyncCreatePDFAndEmail("Quality/IncomingCompoundApprovalRegister","summaryreport", $quality_grp_email,"","Incoming Compound Test Data Report for:".$lastMonth,"Dear Sir/Madam,\n Please find the attached file for the Incoming Compound Test Data Report for :".$lastMonth,true);									
		$aEmail->start();
		exit();						
	}
	...
	$inv_comp_qc		=	@getMySQLData("select *,SUBSTRING_INDEX(tccr.batId, '_',-1) as dispBatId from tbl_compound_ic_qan tcq 
												...
												inner join tbl_component_cpd_recv tccr on tcq.batId = tccr.sno
